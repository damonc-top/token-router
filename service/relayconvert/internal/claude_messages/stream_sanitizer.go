...
func (s *ClaudeStreamSanitizer) ensureOpen(typ string) []ClaudeSanitizedEvent {
	...
	start := dto.ClaudeResponse{
		...
		Index: common.GetPointer(assigned),
		ContentBlock: &dto.ClaudeMediaMessage{
			Type: typ,
		},
	}
	if typ == "text" {
		start.ContentBlock.Text = common.GetPointer("")
	}
	if typ == "thinking" {
		start.ContentBlock.Thinking = common.GetPointer("")
	}
	body, err := common.Marshal(start)
	if err != nil {
		return out
	}
...
func remapClaudeStreamIndex(event *dto.ClaudeResponse, data string, index int) ClaudeSanitizedEvent {
	...
	if err != nil {
		body, mErr := common.Marshal(cloned)
		if mErr != nil {
			return ClaudeSanitizedEvent{Response: cloned, Data: data}
		}
