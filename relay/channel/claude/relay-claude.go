...
func shouldSkipClaudeMessageDeltaUsagePatch(info *relaycommon.RelayInfo) bool {
	...
	return info.HasChannelMeta() && info.ChannelSetting.PassThroughBodyEnabled
}
...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string) *types.NewAPIError {
	...
	if info.HasChannelMeta() && info.ChannelType == constant.ChannelTypeDeepSeek {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		if claudeResponse.Type == "message_start" {
			...
			if claudeResponse.Message != nil {
				...
				if info.HasChannelMeta() && info.IsModelMapped && claudeResponse.Message.Model != info.OriginModelName {
		...
		writeClaudeStreamEvents(c, claudeInfo.SanitizeStreamEvent(&claudeResponse, data))
...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		writeClaudeStreamEvents(c, claudeInfo.CloseSanitizedStream())
	} else if info.RelayFormat == types.RelayFormatOpenAI {
...
func writeClaudeStreamEvents(c *gin.Context, events []relayconvert.ClaudeSanitizedEvent) {
	for _, event := range events {
		if event.Data == "" {
			continue
		}
		helper.ClaudeChunkData(c, event.Response, event.Data)
	}
}
...
func HandleClaudeResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, httpResp *http.Response, data []byte) *types.NewAPIError {
	...
	case types.RelayFormatClaude:
		responseData = data
		if info.HasChannelMeta() && info.IsModelMapped && claudeResponse.Model != info.OriginModelName {
